...
class Pointer:
	...
	def __init__(self,
			addr_:"int", size_:"int",
			# Private:
			_typed_:"list[ALLOCATION_TYPE]|None"=None, _unsafe_free_cb_:"Callable[[],int]|None"=None
	) -> None:
		global _REF_VARS_SPECIAL_SAUCE
	...
	def _get_data_size(self, types_:"list[ALLOCATION_TYPE]") -> "int":
		if types_ in [[ALLOCATION_TYPE.INT_8,], [ALLOCATION_TYPE.U_INT_8,]]:
			return 1 	# 1 byte
		if types_ in [[ALLOCATION_TYPE.INT_16,], [ALLOCATION_TYPE.U_INT_16,]]:
			return 2	# 2 bytes
		if types_ in [[ALLOCATION_TYPE.INT_32,], [ALLOCATION_TYPE.U_INT_32,], [ALLOCATION_TYPE.FLOAT_32,]]:
			return 4
		if types_ in [[ALLOCATION_TYPE.INT_64,], [ALLOCATION_TYPE.U_INT_64,], [ALLOCATION_TYPE.FLOAT_64,]]:
			return 8
		if types_ in [[ALLOCATION_TYPE.BOOL,]]:
			return 1
		if types_ in [[ALLOCATION_TYPE.CHAR,]]:
			return 1
		if types_ in [
				[ALLOCATION_TYPE.INT_8, ALLOCATION_TYPE.PTR,],
				[ALLOCATION_TYPE.U_INT_8, ALLOCATION_TYPE.PTR,],
				[ALLOCATION_TYPE.BOOL, ALLOCATION_TYPE.PTR,],
				[ALLOCATION_TYPE.CHAR, ALLOCATION_TYPE.PTR,],
			]:
			return 1
		if types_ in [
				[ALLOCATION_TYPE.INT_16, ALLOCATION_TYPE.PTR,],
				[ALLOCATION_TYPE.U_INT_16, ALLOCATION_TYPE.PTR,],
			]:
			return 2
		if types_ in [
				[ALLOCATION_TYPE.INT_32, ALLOCATION_TYPE.PTR,],
				[ALLOCATION_TYPE.U_INT_32, ALLOCATION_TYPE.PTR,],
				[ALLOCATION_TYPE.FLOAT_32, ALLOCATION_TYPE.PTR,],
			]:
			return 4
		if types_ in [
				[ALLOCATION_TYPE.INT_64, ALLOCATION_TYPE.PTR,],
				[ALLOCATION_TYPE.U_INT_64, ALLOCATION_TYPE.PTR,],
				[ALLOCATION_TYPE.FLOAT_64, ALLOCATION_TYPE.PTR,],
			]:
			return 8
		raise ValueError("Type not supported.")

	def free(self) -> None:
		if self._unsafe_free_cb_:
			self._unsafe_free_cb_()
		else:
			raise ValueError("No free callback provided.")

	def _validate(self, size_:"int", types_:"list[ALLOCATION_TYPE]|None") -> None:
		if types_ is None:
			return
		if types_[-1] == ALLOCATION_TYPE.PTR:
			return
		assert len(types_) == 1
...
class alloc_handler:
	...
	def __LOCK(self, from_address=None) -> "_CUSTOM_LOCK":
		...
		ptr = alloc(1).unsafe_access(ALLOCATION_TYPE.BOOL)
		ptr.write_bool(False)
		return _CUSTOM_LOCK(ptr)

	def __init__(self, address_book:"str|None"=None):
		self.__mapped:"dict[str,list[ALLOCATION_TYPE]]" = {}
	...
	def __push_address(self, name_:"str", addr_:"int", size_:"int", types_:"list[ALLOCATION_TYPE]") -> None:
	...
	def allocate(self, name_:"str", *types_:"ALLOCATION_TYPE", eq_:"int|bytes|None"=None, ptr_size_:"int|None"=None) -> None:
		if name_ in self.__assigned:
			return
		if len(types_) == 1:
			type_ = types_[0]
			assert type_ == ALLOCATION_TYPE.BOOL or type_ == ALLOCATION_TYPE.INT_8 or type_ == ALLOCATION_TYPE.U_INT_8
			ptr_size_ = 1
			...
			if [*types_,] != [ALLOCATION_TYPE.CHAR, ALLOCATION_TYPE.PTR]:
				raise ValueError(e_msg)
